...
int CRtmpClient::Send( PBYTE pbuf, int nBufLen, int type, DWORD dwTimeStamp, unsigned int nCTOffset )
{
    int nRet = 1;
    if( type == RTMP_PACKET_TYPE_VIDEO )
    {// Send Video
        NaluUnit nalu;
        nRet = ReadOneNaluFromBuf(nalu,pbuf,nBufLen,dwTimeStamp,nCTOffset);
    }
    else
    {// Send Audio
        if( 0 == m_nCacheAudioSampleCount )
            nRet = SendAACPacket(pbuf,nBufLen,dwTimeStamp);
        else
        {
            // mwb 2014.12.08 add
            if( !m_bSendAudioPacket )
            {
                if( ++m_nBufferAudioSampleCount < m_nCacheAudioSampleCount)
                {
                    memcpy(m_aAudioPacketData[m_nWirteAudioPacketIndex].m_abyAudioData,pbuf,nBufLen);
                    m_aAudioPacketData[m_nWirteAudioPacketIndex].m_nDataSize = nBufLen;
                    m_aAudioPacketData[m_nWirteAudioPacketIndex].m_dwPTS = dwTimeStamp;
                    ++m_nWirteAudioPacketIndex;
                    m_nWirteAudioPacketIndex = m_nWirteAudioPacketIndex%AUDIOPACKETARRAYSIZE;
                }
                else
                {
                    memcpy(m_aAudioPacketData[m_nWirteAudioPacketIndex].m_abyAudioData,pbuf,nBufLen);
            ...
            nRet = SendAACPacket(m_aAudioPacketData[m_nReadAudioPacketIndex].m_abyAudioData,m_aAudioPacketData[m_nReadAudioPacketIndex].m_nDataSize,dwTimeStamp);
            ++m_nReadAudioPacketIndex;
            m_nReadAudioPacketIndex = m_nReadAudioPacketIndex%AUDIOPACKETARRAYSIZE;
        }
    }
    return nRet;
}
...
#if 1
int CRtmpClient::ReadOneNaluFromBuf(NaluUnit &nalu, BYTE *pByte, int nBufLen,unsigned int dwVideoTimeStamp,unsigned int dwCTOffset)
{
    int i=0;
    int nRet = 1;
    while(i<nBufLen-4)
            ...
            while( pos < nBufLen -4 )
            {
                ...
                {// 若不等于 00 00 00 01
                    ...
                    {// 若等于 00 00 01，则代表找到了VCL的NALU
                        ...
                        nRet = SendH264Packet(nalu.data,nalu.size,bKeyframe,dwVideoTimeStamp,dwCTOffset);
                        return nRet;
                    }
                }
                pos++;
...
int CRtmpClient::SendH264Packet(PBYTE pBuf,unsigned int size,bool bIsKeyFrame,unsigned int dwVideoTimeStamp,unsigned int dwCTOffset)
{
    if(pBuf == NULL && size<11)
    {
        return false;
    }
    ...
    {
        body[i++] = dwCTOffset>>16;
        body[i++] = dwCTOffset>>8;
        body[i++] = dwCTOffset&0xff;
    }
    ...
    // NALU data
    memcpy(&body[i],pBuf,size);

    int nRet = SendRTMPPacket(body,i+size,RTMP_PACKET_TYPE_VIDEO,dwVideoTimeStamp );
    delete[] body;
    return nRet;
}
...
#ifdef MYSELF_USE_LIBAAC_ENCODER
    unsigned char *body = new unsigned char[size-5]; // 5 = 7-2 :7 AAC Header 2: rtmp audio tag
    body[0] = 0xaf;
    body[1] = 0x01;
    memcpy(&body[2],&pBuf[7],size-7);  // 拷贝AAC Raw Data
    int nRet = SendRTMPPacket(body,size-5,RTMP_PACKET_TYPE_AUDIO,dwAudioTimeStamp);
#else
    unsigned char *body = new unsigned char[size-7]; // 7 = 9-2 :9 AAC Header + 2 byte CRC 2: rtmp audio tag
    body[0] = 0xaf;
    body[1] = 0x01;
    memcpy(&body[2],&pBuf[9],size-9);
    int nRet = SendRTMPPacket(body,size-7,RTMP_PACKET_TYPE_AUDIO,dwAudioTimeStamp);
...
void CRtmpClient::SetAudioDelay(DWORD dwAudioDelay)
{
